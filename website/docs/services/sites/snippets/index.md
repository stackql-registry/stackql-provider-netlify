...
<tr><td><b>Id</b></td><td><CopyableCode code="netlify.sites.snippets" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO netlify.sites.snippets (
id,
site_id,
title,
general,
...
- name: snippets
  props:
    - name: site_id
      value: "{{ site_id }}"
      description: Required parameter for the snippets resource.
    - name: id
      value: {{ id }}
    - name: site_id
      value: "{{ site_id }}"
...
```sql
REPLACE netlify.sites.snippets
SET 
id = {{ id }},
site_id = '{{ site_id }}',
title = '{{ title }}',
...
```sql
DELETE FROM netlify.sites.snippets
WHERE site_id = '{{ site_id }}' --required
AND snippet_id = '{{ snippet_id }}' --required
;
```
